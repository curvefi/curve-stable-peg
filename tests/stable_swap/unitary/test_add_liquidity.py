...
def test_add_with_slippage(bob, swap, pool_token, decimals, coins, n_coins):
    amounts = [10 ** i for i in decimals]
    amounts[0] = int(amounts[0] * 0.99)
    amounts[1] = int(amounts[1] * 1.01)
    value = amounts[0] if ETH_ADDRESS in coins else 0
...
def test_add_one_coin(
    bob, swap, coins, pool_token, initial_amounts, base_amount, idx, n_coins
):
    amounts = [0] * n_coins
...
def test_wrong_eth_amount(bob, swap, coins, pool_token, initial_amounts, n_coins):
    # for ETH pools, tests sending too much ETH
    # for non-ETH pools, tests that function is nonpayable
    value = initial_amounts[0] - 1 if ETH_ADDRESS in coins else 1
    with brownie.reverts():
        swap.add_liquidity(initial_amounts, 0, {"from": bob, "value": value})
