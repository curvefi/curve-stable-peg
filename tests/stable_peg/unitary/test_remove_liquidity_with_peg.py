...
def test_remove_liquidity_equal(swap, pool_token, peg, pegged, alice, peg_keeper, admin):
    ...
    amounts = swap.remove_liquidity(pool_token.balanceOf(alice), [0, 0], {"from": alice}).return_value

    assert balances == [swap.balances(0) + amounts[0], swap.balances(1) + amounts[1]]
    assert real_balances == [peg.balanceOf(swap) + amounts[0], pegged.balanceOf(swap) + amounts[1]]


def test_remove_liquidity_more_peg(swap, pool_token, peg, pegged, alice, peg_keeper, balance_change_after_withdraw):
    """ Remove [2 * x, x] """
...
def test_remove_liquidity_more_pegged(swap, pool_token, alice, balance_change_after_provide):
    """ Remove [x, 2 * x] """
    token_balance = pool_token.balanceOf(alice)
    # Balances in the pool are equal
