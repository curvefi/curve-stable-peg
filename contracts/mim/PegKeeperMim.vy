...
PROFIT_PRECISION: constant(uint256) = 10 ** 5
callers_part: public(uint256)
...
receivers_profit: public(uint256)

admin_actions_deadline: public(uint256)


@external
def __init__(_pool: address, _receiver: address, _min_asymmetry: uint256, _callers_part: uint256):
    """
    ...
    self.callers_part = _callers_part
...
@external
def update(_profit_receiver: address = msg.sender) -> uint256:
    """
    @notice Provide or withdraw coins from the pool to stabilize it
    @param _profit_receiver Address of profit receiver
    @return Amount of profit received by the caller of update
    """
    ...
    caller_profit: uint256 = lp_amount * self.callers_part / PROFIT_PRECISION
    self.receivers_profit += lp_amount - caller_profit
    CurvePool(self.pool).transfer(_profit_receiver, caller_profit)
...
@external
def set_new_callers_part(_new_callers_part: uint256):
    """
    @notice Set new update caller's part
    @param _new_callers_part Part with PROFIT_PRECISION
    """
    assert msg.sender == self.admin  # dev: only admin
    assert _new_callers_part <= PROFIT_PRECISION  # dev: bad part value

    self.callers_part = _new_callers_part
...
def withdraw_profit() -> uint256:
    ...
    """
    lp_amount: uint256 = self.receivers_profit
    CurvePool(self.pool).transfer(self.receiver, lp_amount)
    self.receivers_profit = 0
