...
@license MIT
@author Curve.Fi
@notice Peg Keeper for metapool with equal decimals of coins
"""
...
METAPOOL: immutable(address)
META_I: immutable(uint256)
...
@external
def __init__(_pool: address, _index: uint256, _meta_index: uint256, _receiver: address, _caller_share: uint256):
    """
    @notice Contract constructor
    @param _pool Contract pool address
    @param _index Index of the pegged
    @param _meta_index Index of the metapool token
    ...
    METAPOOL = CurvePool(_pool).coins(_meta_index)
    META_I = _meta_index
